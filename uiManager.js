class UIManager {
    ...
    initializeEventListeners() {
        ...
        try {
            ...
            const manageArchiveBtn = document.getElementById('manageArchiveBtn');
            if (manageArchiveBtn) {
                manageArchiveBtn.addEventListener('click', () => {
                    this.showManageArchive();
                });
            }

            // Save Archive Title Button
            const saveArchiveTitleBtn = document.getElementById('saveArchiveTitleBtn');
            if (saveArchiveTitleBtn) {
                saveArchiveTitleBtn.addEventListener('click', () => {
                    this.saveArchiveTitle();
                });
            }

            // Confirm Delete Archive Button
            const confirmDeleteArchiveBtn = document.getElementById('confirmDeleteArchiveBtn');
            if (confirmDeleteArchiveBtn) {
                confirmDeleteArchiveBtn.addEventListener('click', () => {
                    this.deleteArchive();
                });
            }
            ...
            document.addEventListener('click', (e) => {
                ...
                // Archive edit button in manage modal
                if (e.target.closest('.archive-edit-btn')) {
                    const editBtn = e.target.closest('.archive-edit-btn');
                    if (editBtn && editBtn.dataset.id) {
                        const archiveId = parseInt(editBtn.dataset.id);
                        this.editArchive(archiveId);
                    }
                }
                
                // Archive delete button in manage modal
                if (e.target.closest('.archive-delete-btn')) {
                    const deleteBtn = e.target.closest('.archive-delete-btn');
                    if (deleteBtn && deleteBtn.dataset.id) {
                        const archiveId = parseInt(deleteBtn.dataset.id);
                        this.confirmDeleteArchive(archiveId);
                    }
                }
            });
    ...
    // Show manage archive modal
    showManageArchive() {
        const archiveList = document.getElementById('archiveList');
        if (!archiveList) return;
        
        const archives = this.budgetManager.getAllArchives();
        
        if (archives.length === 0) {
            archiveList.innerHTML = '<p>No archived budgets found</p>';
            this.showModal('manageArchiveModal');
            return;
        }
        
        let html = '';
        archives.forEach(archive => {
            const startDate = this.transactionManager.formatDate(archive.startDate);
            const endDate = archive.endDate ? this.transactionManager.formatDate(archive.endDate) : 'Present';
            const totalBudget = this.transactionManager.formatCurrency(archive.totalBudget);
            const totalSpent = this.transactionManager.formatCurrency(archive.totalSpent);
            
            html += `
                <div class="archive-select-item">
                    <div class="archive-select-info">
                        <h4>${archive.name}</h4>
                        <div class="archive-select-dates">${startDate} - ${endDate}</div>
                        <div>Budget: ${totalBudget} | Spent: ${totalSpent}</div>
                    </div>
                    <div class="archive-select-actions">
                        <button class="archive-edit-btn" data-id="${archive.id}" title="Edit Title">
                            ✏️ Edit
                        </button>
                        <button class="archive-delete-btn" data-id="${archive.id}" title="Delete Archive">
                            🗑️ Delete
                        </button>
                    </div>
                </div>
    ...
    editArchive(archiveId) {
        const archive = this.budgetManager.getArchiveById(archiveId);
        if (!archive) {
        ...
        // Fill edit form
        const editArchiveId = document.getElementById('editArchiveId');
        const editArchiveTitle = document.getElementById('editArchiveTitle');
        const editArchivePeriod = document.getElementById('editArchivePeriod');
        const editArchiveTotalBudget = document.getElementById('editArchiveTotalBudget');
        const editArchiveTotalSpent = document.getElementById('editArchiveTotalSpent');
        const editArchiveSaved = document.getElementById('editArchiveSaved');
        
        if (editArchiveId) editArchiveId.value = archiveId;
        if (editArchiveTitle) editArchiveTitle.value = archive.name;
        
        const startDate = this.transactionManager.formatDate(archive.startDate);
        const endDate = archive.endDate ? this.transactionManager.formatDate(archive.endDate) : 'Present';
        const totalBudget = this.transactionManager.formatCurrency(archive.totalBudget);
        const totalSpent = this.transactionManager.formatCurrency(archive.totalSpent || 0);
        const savings = this.transactionManager.formatCurrency(archive.savings || 0);
        
        if (editArchivePeriod) editArchivePeriod.textContent = `${startDate} - ${endDate}`;
        if (editArchiveTotalBudget) editArchiveTotalBudget.textContent = totalBudget;
        if (editArchiveTotalSpent) editArchiveTotalSpent.textContent = totalSpent;
        if (editArchiveSaved) editArchiveSaved.textContent = savings;
        
        this.hideAllModals();
        setTimeout(() => {
            this.showModal('editArchiveModal');
        }, 300);
    }
    
    // Save archive title
    saveArchiveTitle() {
        const archiveId = parseInt(document.getElementById('editArchiveId')?.value || '0');
        const newTitle = document.getElementById('editArchiveTitle')?.value.trim();
        ...
        const success = this.budgetManager.editArchiveTitle(archiveId, newTitle);
        if (success) {
            this.hideAllModals();
            this.updateUI();
            alert('Archive title updated successfully!');
            
            // Refresh the archive screen if we're on it
    ...
    confirmDeleteArchive(archiveId) {
        const archive = this.budgetManager.getArchiveById(archiveId);
        if (!archive) {
            alert('Archive not found!');
            return;
        ...
        const deleteArchiveId = document.getElementById('deleteArchiveId');
        const deleteArchiveName = document.getElementById('deleteArchiveName');
        const deleteArchivePeriod = document.getElementById('deleteArchivePeriod');
        const deleteArchiveTransactions = document.getElementById('deleteArchiveTransactions');
        const deleteArchiveTotalSpent = document.getElementById('deleteArchiveTotalSpent');
        
        if (deleteArchiveId) deleteArchiveId.value = archiveId;
        if (deleteArchiveName) deleteArchiveName.textContent = archive.name;
        
        const startDate = this.transactionManager.formatDate(archive.startDate);
        const endDate = archive.endDate ? this.transactionManager.formatDate(archive.endDate) : 'Present';
        if (deleteArchivePeriod) deleteArchivePeriod.textContent = `${startDate} - ${endDate}`;
        
        const transactions = archive.transactions?.length || 0;
        if (deleteArchiveTransactions) deleteArchiveTransactions.textContent = transactions;
        
        const totalSpent = this.transactionManager.formatCurrency(archive.totalSpent || 0);
        if (deleteArchiveTotalSpent) deleteArchiveTotalSpent.textContent = totalSpent;
        
        this.hideAllModals();
        setTimeout(() => {
            this.showModal('deleteArchiveModal');
        }, 300);
    }
    ...
    deleteArchive() {
        ...
        const success = this.budgetManager.deleteArchive(archiveId);
        if (success) {
            this.hideAllModals();
            this.updateUI();
